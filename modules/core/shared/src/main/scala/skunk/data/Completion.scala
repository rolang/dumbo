...
object Completion {
  ...
  case class Multiple(list: List[Completion]) extends Completion

  /**
   * Instead of crashing (which breaks the protocol and hangs everything) let's
   * allow for unknown completion messages and print out a stacktrace on
   * construction.
   */
  case class Unknown(text: String) extends Completion {
    new SkunkException(
      sql = None,
      message =
        s"Just constructed an unknown completion '$text'.  Note that your program has not crashed. This message is here to annoy you.",
      hint = Some(
        "Please open an issue, or open a PR adding a case in Completion.scala and a parser in CommandComplete.scala"
      ),
